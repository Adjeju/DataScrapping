...
class MoyoSpiderMiddleware:
    ...
    def process_spider_output(self, response, result, spider):
        for i in result:
            if(isinstance(i, MoyoItem)):
                yield JsonRequest(
                    url=f'http://localhost:3000/items?name={i["model"]}&price={i["price"]}',
                    method='GET',
                    callback=spider.update_item,
                    errback=spider.create_item
                )
            yield i
...
class MoyoDownloaderMiddleware:
    # Not all methods need to be defined. If a method is not defined,
